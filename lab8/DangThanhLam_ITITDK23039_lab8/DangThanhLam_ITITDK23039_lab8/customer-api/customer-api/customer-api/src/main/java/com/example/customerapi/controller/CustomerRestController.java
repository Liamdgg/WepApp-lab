...
public class CustomerRestController {
    ...
    public ResponseEntity<Map<String, Object>> getAllCustomers(
        ...
        org.springframework.data.domain.Page<CustomerResponseDTO> customerPage = customerService.getAllCustomers(page, size, sortBy, sortDir);
    ...
    public ResponseEntity<List<CustomerResponseDTO>> getCustomersByStatus(@PathVariable String status) {
        ...
        try {
            CustomerStatus enumStatus = CustomerStatus.valueOf(normalized);
            List<CustomerResponseDTO> customers = customerService.getCustomersByStatus(enumStatus.name());
            return ResponseEntity.ok(customers);
        } catch (IllegalArgumentException ex) {
            return ResponseEntity.badRequest().body(List.of()); // or ResponseEntity.badRequest().build();
        }
    }
